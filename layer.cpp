...
void fully_connected_layer::init(layer_data data, const function<float(float)> &activationFunct,
...
void fully_connected_layer::feedforward(int _, vector<vector<vector<float>>> &a,
...
void fully_connected_layer::backprop(int _, vector<vector<vector<float>>> &delta, vector<vector<vector<float>>> &activations,
                                     vector<vector<vector<float>>> &derivative_z) {
    for (int neuron = 0; neuron < n_out; neuron++) updateB[neuron] += delta[0][0][neuron];
    ...
    vector<vector<vector<float>>> newDelta(1, vector<vector<float>> (1, vector<float> (n_in, 0)));

    for (int neuron = 0; neuron < n_out; neuron++) {
        for (int previous_neuron = 0; previous_neuron < n_in; previous_neuron++) {
            newDelta[0][0][previous_neuron] += delta[0][0][neuron] * weights[neuron][previous_neuron];
            if (neuron == n_out - 1) newDelta[0][0][previous_neuron] *= derivative_z[0][0][previous_neuron];
        }
    }
    delta = newDelta;
}
...
void convolutional_layer::init(layer_data data, const function<float(float)> &activationFunct,
                               const function<float(float)> &activationFunctPrime,
                               const function<float(float, float)> &costFunctPrime) {
    ...
    updateW = vector<vector<vector<float>>>(feature_maps, vector<vector<float>>(receptive_field_length,
                                                                                vector<float>(receptive_field_length,
                                                                                              0)));
}

void convolutional_layer::feedforward(int previous_feature_maps, vector<vector<vector<float>>> &a,
                                      vector<vector<vector<float>>> &derivative_z) {

    vector<vector<vector<float>>> z(feature_maps *previous_feature_maps, vector<vector<float>> (n_out.y, vector<float>(
            n_out.x, 0)));
    vector<vector<vector<float>>> new_a = z;
    derivative_z = z;
                ...
                for (int x = 0; x < n_out.x; x++) {
                    for (int kernel_y = 0; kernel_y < receptive_field_length; kernel_y++) {
                        for (int kernel_x = 0; kernel_x < receptive_field_length; kernel_x++) {
                            ...
                            z[previous_map * feature_maps + map][y][x] *= a[previous_map][y * stride_length + kernel_y][
                                    x * stride_length + kernel_x];
                        }
                    }
                    z[previous_map * feature_maps + map][y][x] += biases[map];
                    new_a[previous_map * feature_maps + map][y][x] = activationFunct(
                            z[previous_map * feature_maps + map][y][x]);
                    derivative_z[previous_map * feature_maps + map][y][x] = activationFunctPrime(
                            z[previous_map * feature_maps + map][y][x]);
                }
            }
        }
...
void convolutional_layer::backprop(int previous_feature_maps, vector<vector<vector<float>>> &delta,
    ...
    for (int map = 0; map < feature_maps; map++) {
        for (int y = 0; y < n_out.y; y++) {
            for (int x = 0; x < n_out.x; x++) {
                for (int kernel_y = 0; kernel_y < receptive_field_length; kernel_y++) {
                    for (int kernel_x = 0; kernel_x < receptive_field_length; kernel_x++) {
                        updateW[map][kernel_y][kernel_x] += activations[map / previous_feature_maps][y * stride_length + kernel_y][x * stride_length + kernel_x] * delta[map][kernel_y][kernel_x];
                    }
                }
            }
        }
    ...
    vector<vector<vector<float>>> newDelta(feature_maps, vector<vector<float>> (n_in.y, vector<float> (n_in.y, 0)));
    ...
    for (int map = 0; map < feature_maps; map++) {
        for (int y = 0; y < n_out.y; y++) {
            for (int x = 0; x < n_out.x; x++) {
                for (int kernel_y = 0; kernel_y < receptive_field_length; kernel_y++) {
                    for (int kernel_x = 0; kernel_x < receptive_field_length; kernel_x++) {
                        newDelta[map / previous_feature_maps][y * stride_length + kernel_y][x * stride_length + kernel_x] += delta[map][y][x] * weights[map][kernel_y][kernel_x] *
                                                                 derivative_z[map / previous_feature_maps][
                                                                         y * stride_length + kernel_y][
                                                                         x * stride_length + kernel_x];
                    }
                }
            }
    ...
    for (int map = 0; map < feature_maps; map++) {
        for (int kernel_y = 0; kernel_y < receptive_field_length; kernel_y++) {
            for (int kernel_x = 0; kernel_x < receptive_field_length; kernel_x++) {
                ...
                weightsVelocity[map][kernel_y][kernel_x] =
                        params.momentum_coefficient * weightsVelocity[map][kernel_y][kernel_x] -
                        (params.convolutional_weights_learning_rate / params.mini_batch_size / (n_out.x * n_out.y) *
                         stride_length * stride_length) * updateW[map][kernel_y][kernel_x];
                weights[map][kernel_y][kernel_x] = (1 -
                                                    params.convolutional_weights_learning_rate / (n_out.x * n_out.y) *
                                                    stride_length * stride_length * params.L2_regularization_term /
                                                    params.training_data_size) * weights[map][kernel_y][kernel_x] +
                                                   weightsVelocity[map][kernel_y][kernel_x];
            }
        }
    }
...
void max_pooling_layer::init(layer_data data, const function<float(float)> &activationFunct,
                             const function<float(float)> &activationFunctPrime,
                             const function<float(float, float)> &costFunctPrime) {
    this->n_in = data.n_in;
...
void max_pooling_layer::feedforward(int previous_feature_maps, vector<vector<vector<float>>> &a,
                                    vector<vector<vector<float>>> &derivative_z) {
    vector<vector<vector<float>>> z(previous_feature_maps, vector<vector<float>>(n_out.y, vector<float>(n_out.x,
...
void max_pooling_layer::backprop(int previous_feature_maps, vector<vector<vector<float>>> &delta,
                                 vector<vector<vector<float>>> &activations,
                                 vector<vector<vector<float>>> &derivative_z) {
    vector<vector<vector<float>>> newDelta(feature_maps, vector<vector<float>> (n_in.y, vector<float> (n_in.y, 0)));
                ...
                for (int kernel_y = 0; kernel_y < summarized_region_length; kernel_y++) {
                    for (int kernel_x = 0; kernel_x < summarized_region_length; kernel_x++) {
                        if (activations[previous_map][y + kernel_y][x + kernel_x] == derivative_z[previous_map][y][x]) {
                            newDelta[previous_map][y + kernel_y][x + kernel_x] += delta[previous_map][y][x] * derivative_z[previous_map][y][x];
                        }
                    }
                }
            }
...
void flatten_layer::feedforward(int previous_feature_maps, vector<vector<vector<float>>> &a,
    ...
    assert(previous_feature_maps * n_in.x * n_in.y == n_out.x);
...
void flatten_layer::backprop(int previous_feature_maps, vector<vector<vector<float>>> &delta,
                             vector<vector<vector<float>>> &activations, vector<vector<vector<float>>> &derivative_z) {

    vector<vector<vector<float>>> newDelta(feature_maps, vector<vector<float>> (n_in.y, vector<float> (n_in.x, 0)));

    for (int map = 0; map < feature_maps; map++) {
        for (int y = 0; y < n_in.y; y++) {
            for (int x = 0; x < n_in.x; x++) {
                newDelta[map][y][x] += delta[0][0][map * n_in.y * n_in.x + y * n_in.x + x];
            }
        }
    }
...
void input_layer::backprop(int previous_feature_maps, vector<vector<vector<float>>> &delta, vector<vector<vector<float>>> &activations,
                           vector<vector<vector<float>>> &z) {}
